...
def build_result_matrix(model, window_size, output_path, image_path, step_size):
    print('building result matrix:', image_path)
    image = cv2.imread(image_path)
    result = window(
        image, 
        build_model_fnc(model),
...
window_size, out_name, img_path = images_list[14]

build_result_matrix(
    vgg_cnn,
    window_size,
    'COU4_results.npy',
    full_images_dir + '/' + img_path,
    10
)